...
class EntityResolver:
    ...
    async def find_duplicates(
        ...
        2. Exact normalized name match (different source_name)
        3. Fuzzy name match above threshold
        """
    ...
    async def _name_matches(
        ...
        Fetches cross-source entity pairs of the same type, then applies
        fuzzy matching in Python (SequenceMatcher is CPU-bound, not SQL).
        """
        ...
        sql = text(f"""
            SELECT id, name, source_name, entity_type, identifier
            FROM entities
        ...
        for etype, entities in by_type.items():
            # Compare cross-source pairs only
            for i, a in enumerate(entities):
                for b in entities[i + 1:]:
                    if a.source_name == b.source_name:
                        continue  # same source, already deduped by UUID

                    score = _fuzzy_score(a.name, b.name)
