...
from okeanus.schema.vessel import VesselObservation, VesselObservationRead
...
async def list_vessels(
    ...
    """Query vessel positions — latest observation per MMSI."""
    # Use DISTINCT ON to get one row per vessel (the most recent)
    stmt = (
    ...
    if mmsi:
        stmt = stmt.where(VesselObservation.mmsi == mmsi)
    if bbox:
        w, s, e, n = [float(x) for x in bbox.split(",")]
        stmt = stmt.where(
            VesselObservation.geometry.ST_Intersects(ST_MakeEnvelope(w, s, e, n, 4326))
        )
    ...
    async with async_session_factory() as session:
        ...
        features = [VesselObservationRead.model_validate(r).to_feature().model_dump() for r in rows]
...
async def get_vessel(mmsi: int) -> Feature:
    ...
    async with async_session_factory() as session:
        ...
        return VesselObservationRead.model_validate(row).to_feature()
