...
class GlobalFishingWatchAdapter(BaseAdapter):
    ...
    async def fetch(
        ...
        **params: Any,
        ...
        """Fetch fishing events or vessel tracks within bbox and time range.

        Pass ``endpoint`` in params: 'events' (default), 'vessels'.
        """
        if not self._api_key:
            logger.warning("GFW adapter requires api_key (free at globalfishingwatch.org)")
            return []
        ...
        return await self._fetch_events(bbox, time_start, time_end, event_type, limit)
    ...
    async def _fetch_events(
        ...
        w, s, e, n = bbox
        body: dict[str, Any] = {
            "datasets": [f"public-global-{event_type}-events:latest"],
        ...
        for event in entries:
            ...
            observations.append({
                ...
                "payload": {
                    ...
                    "duration_hours": event.get("durationHrs"),
                    "event_end": event.get("end"),
                    "distances_from_shore_km": event.get("distances", {}).get(
                        "startDistanceFromShoreKm"
                    ),
                },
            })

        logger.info("GFW returned %d %s events", len(observations), event_type)
        return observations

    async def _fetch_vessel_search(self, query: str) -> list[dict[str, Any]]:
        ...
        for v in vessels:
            observations.append({
                ...
                "source_id": f"gfw-vessel-{v.get('id', '')}",
                "source_name": "Global Fishing Watch",
                "mmsi": v.get("ssvid"),
                "quality_score": None,
                "payload": {
